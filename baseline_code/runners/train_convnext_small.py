#!/usr/bin/env python
"""
ConvNeXt-Small Hybrid Training - Fold 0
Optimized parameters:
  - Weight Decay: 0.00012 (+41% increase for 50M model)
  - Learning Rate: 0.0004 (baseline, more stable)
"""
...
def train(config):
    print("="*80)
    print("🚀 ConvNeXt-Small Hybrid Training - Fold 0")
    print("="*80)
    print("\n📋 Configuration:")
    ...
    if config.get("wandb"):
        ...
        exp_name = f"convnext_small_optimized_fold0_{datetime.now().strftime('%Y%m%d_%H%M')}"
        project = "convnext-small-ocr-fold0"
        ...
        logger = Logger(
            ...
            config=dict(config),
            log_model=False,
            tags=["convnext_small", "optimized", "fold0", "progressive_res", "wd_0.00012"],
        )
    else:
