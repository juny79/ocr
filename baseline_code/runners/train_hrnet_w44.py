#!/usr/bin/env python
"""
HRNet-W44 Hybrid Training - Fold 0

AGGRESSIVE Optimized parameters leveraging HRNet architecture:
  - Model: HRNet-W44 (57M parameters - parallel multi-resolution)
  - Weight Decay: 0.000082 (LOWER than Tiny's 0.000085)
  - Learning Rate: 0.00045 (SAME as Tiny's success)
  
HRNet-Specific Strategy:
  - HRNet's parallel multi-resolution branches provide IMPLICIT regularization
  - Continuous multi-scale fusion prevents overfitting structurally
  - 57M params but architectural advantage allows lighter regularization
  - Can use Tiny's successful LR (0.00045) due to stable multi-resolution training
  - Weight decay LOWER than Tiny (0.000082 < 0.000085) - trust the architecture
  
Key Insight:
  - ConvNeXt (sequential) needs explicit regularization
  - HRNet (parallel fusion) has built-in regularization
  - Don't over-regularize HRNet like we did with ConvNeXt-Small
"""
...
def train(config):
    print("="*80)
    print("🚀 HRNet-W44 Hybrid Training - Fold 0")
    print("="*80)
    print("\n📋 Configuration:")
    print(f"  • Model: HRNet-W44 (~67M parameters)")
    ...
    print("\n💡 Strategy:")
    print(f"  • Moderate regularization (wd=0.0001)")
    print(f"  • Avoiding ConvNeXt-Small's over-regularization mistake")
    print(f"  • Following ConvNeXt-Tiny's successful approach")
    print("="*80 + "\n")
    ...
    if config.get("wandb"):
        ...
        exp_name = f"hrnet_w44_optimized_fold0_{datetime.now().strftime('%Y%m%d_%H%M')}"
        project = "hrnet-w44-ocr-fold0"
        ...
        logger = Logger(
            ...
            config=dict(config),
            log_model=False,
            tags=["hrnet_w44", "optimized", "fold0", "progressive_res", "wd_0.0001"],
        )
    else:
