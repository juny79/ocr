...
## 해상도 최적화 및 파라미터 튜닝 (1280px → 1024px)
...
### 1.1 목표
- 기존 1280×1280 해상도를 **1024×1024로 최적화** (속도/성능 균형)
- DBNet 텍스트 감지 모델의 효율성 개선
- 최적의 하이퍼파라미터 조합 도출

### 1.2 실험 기간
- **시작:** 1280px 기준선 분석
- **진행:** 1024px 해상도로 조정 및 파라미터 최적화
...
### 3.1 Phase 1: 초기 1280px 기준선 (Baseline)
...
### 3.2 Phase 2: 해상도 최적화 + 파라미터 개선

#### 2.1단계: 1280px → 1024px 조정
**동기:**
- 학습 속도 개선 (약 1.6배 빠름)
- 메모리 효율성 향상
- 과적합 위험 감소
...
```yaml
# 변경 사항
Resolution: 1280×1280 → 1024×1024  (-40% 픽셀 수)
Batch Size: 4 (유지 - GPU 메모리 여유 확보)
